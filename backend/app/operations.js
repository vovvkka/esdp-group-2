...
router.get("/reports", auth, async (req, res) => {
   ...
   query.title = {
      $in: [config.operations.purchase, config.operations.returnPurchase],
   };
   try {
      const operations = await Operation.aggregate([
         { $match: query },
         {
            $group: {
               ...
               sales: {
                  $sum: {
                     $cond: [
                        { $eq: ["$title", config.operations.purchase] },
                        "$additionalInfo.amountOfMoney",
                        0,
                     ],
                  },
               },
               returns: {
                  $sum: {
                     $cond: [
                        { $eq: ["$title", config.operations.returnPurchase] },
                        "$additionalInfo.amountOfMoney",
                        0,
                     ],
                  },
               },
               salesPurchasePriceTotal: {
                  $sum: {
                     $cond: [
                        { $eq: ["$title", config.operations.purchase] },
                        "$additionalInfo.purchasePriceTotal",
                        0,
                     ],
                  },
               },
               returnsPurchasePriceTotal: {
                  $sum: {
                     $cond: [
                        { $eq: ["$title", config.operations.returnPurchase] },
                        "$additionalInfo.purchasePriceTotal",
         ...
         },
         {
            $addFields: {
               totalProfit: {
                  $subtract: ["$totalSales", "$totalPurchasePriceTotal"],
               },
            },
         },
         {
...
router.get("/report/:id", auth, async (req, res) => {
   ...
   try {
      const shift = await Shift.findById(shiftId)
         .select("cashier shiftNumber createdAt isActive")
         .populate("cashier", "displayName");

      if (shift) {
         if (!shift.isActive) {
            return res
               .status(403)
               .send({ message: "Operation can not be done!" });
         }
      } else {
...
router.get("/report-z/:id", auth, async (req, res) => {
   ...
   try {
      const shift = await Shift.findById(shiftId)
         .select("cashier shiftNumber updatedAt isActive")
         .populate("cashier", "displayName");
      ...
      const reportSales = await Operation.aggregate([
         ...
         { $match: { "shiftInfo.shiftNumber": { $lte: shift.shiftNumber } } },
         {
            $group: {
               _id: null,
               total: { $sum: "$additionalInfo.amountOfMoney" },
               count: { $sum: 1 },
            },
         },
      ]);
      ...
      if (title === config.operations.purchase) {
         ...
         const completePurchaseInfo = await Promise.all(
            purchaseInfo.map(async (i) => {
               ...
               await Product.findByIdAndUpdate(i._id, {
                  amount: item.amount - i.quantity,
               });
               return {
                  ...i,
                  price: item.price,
         ...
         await cash.save();
         let profit = 0;
         completePurchaseInfo.forEach(
            (i) => (profit += i.quantity * i.purchasePrice)
         );
         ...
         const purchase = await Operation.findOne({
            operationNumber: checkNumber,
         });
         if (!purchase || purchase.title !== config.operations.purchase) {
            return res.status(404).send({ message: "Wrong receipt number" });
         }
         const item = purchase.additionalInfo.completePurchaseInfo.filter(
            (i) => i.barcode === barcode
         );
         ...
         await res.send({
            total: Math.round(
               item[0].price -
                  (item[0].price * quantity * item[0].discount) / 100
            ),
         });
         ...
         const purchaseInfo = purchase.additionalInfo.completePurchaseInfo.map(
            (i) => {
         ...
         await Operation.findByIdAndUpdate(purchase._id, {
            ...purchase,
            completePurchaseInfo: purchaseInfo,
         });
