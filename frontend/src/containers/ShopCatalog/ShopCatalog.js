...
const ShopCatalog = () => {
    ...
    useEffect(() => {
        setOptions([{_id: '123', title: 'Все товары', value: '123',id:'123',pId:0,isLeaf:true}, ...categories]);
    }, [categories]);

    useEffect(()=>{
        if(state==='123'){
            dispatch(fetchProductsSearch('?page=' + currentPage + '&user=true'));
        }else {
            dispatch(fetchProductsSearch('?page=' + currentPage + '&user=true&category=' + state));
        }
    ...
    const onLoadData = async ({id}) => {
        const response = await axiosApi('/categories/?node=' + id + '&user=true');
    ...
    const filterByPrice = query =>{
        if(state==='123'){
            dispatch(fetchProductsSearch('?page=' + currentPage+ '&price=' + query));
        }else {
            dispatch(fetchProductsSearch('?page=' + currentPage + '&category=' + state+ '&price=' + query));
        }
    };
    const filterByDate = query =>{
        if(state==='123'){
            dispatch(fetchProductsSearch('?page=' + currentPage+ '&date=' + query));
        }else {
            dispatch(fetchProductsSearch('?page=' + currentPage + '&category=' + state+ '&date=' + query));
        }
    };
    return (
            ...
            <div>
                <div className='container'>
                    ...
                    <div className="location">
                        Главная —{" "}
                        <span className="location__page"> Каталог</span>
                    </div>
                    ...
                    <Stack mt='25px'>
                        <div style={{display:'flex'}}>
                            <TreeSelect
                                ...
                                treeData={options}
                            />
                            <Filter filterByDate={filterByDate} filterByPrice={filterByPrice}/>
                        </div>
                        {loading ? <Spinner/> : (
                            <div>
                                ...
                                {products.docs?.length>0 ?
                                        ...
                                        <Typography sx={{justifySelf: 'start'}}>Нет в наличии</Typography>
                                }
                            </div>
                        )}
